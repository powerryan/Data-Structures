...
public:
  ...
  // (use q.push(node)).
  //
  // When accessing items from the queue, top() to read it, and
  // pop() to remove it.
  //
  // The first item removed should form a left node, the second item
  // should form the right.
  ...
  void Traverse(shared_ptr<freq_info> current, shared_ptr<freq_info> parent, map<char,string>& mymap, string path);
